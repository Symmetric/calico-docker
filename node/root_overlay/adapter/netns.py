...
def set_up_endpoint(ip, cpid, in_container=False, veth_name=VETH_NAME, proc_alias=PROC_ALIAS):
    ...
    try:
        ...
        if in_container:
            _log.debug('In container, linking to root namespace')
            try:
        ...
        _log.debug('Initial netns %d "ip link" output:\n%s',
                   cpid,
                   check_call("ip netns exec %s ip link" % cpid, shell=True))
        ...
        _log.debug('"ip link" output in netns %s:\n:%s',
                   cpid,
                   check_output("ip netns exec %s ip link" % cpid, shell=True))
